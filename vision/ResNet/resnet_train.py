...
from utils import read_json_path, read_json, train_one_epoch, evaluate, save_best_model
from log import Logger
import torchvision.models.resnet

weight_path = '../../autodl-tmp/1-ResNet/bird-101'
...
def main(args, kfold, logger):
    ...
    if args.freeze_layers:
        ...
        for name, module in model.named_modules():
            if name in ['layer1', 'layer2', 'layer3', 'layer4']:
                for bottleneck_name, bottleneck in module.named_modules():
                    if hasattr(bottleneck, 'se'):
                        for param in bottleneck.seblock.parameters():
                            param.requires_grad = True
    ...
    for epoch in range(args.epochs):
        ...
        logger.info('%d epoch train mean loss: %.4f ' % (epoch, mean_loss))
        logger.info('%d epoch train mean acc: %.4f ' % (epoch, mean_acc))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='./resnet101-pre.pth',
